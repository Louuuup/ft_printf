...
/*   Updated: 2023/03/28 16:16:32 by ycyr-roy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	int_disp1(int nb_len, t_data *data)
{
...
// TO TEST: Change pre and wtdh so data.pre = data-len. !!!!!!
// Handles Precisions (PRE)
int	int_disp2(long long nb, t_data *data)
{
	if (data->wdh < data->pre)
		return (ft_flood(data->pre - int_len(nb), '0', data->len));
	return (NO_ERROR);
}

// Handles '-' and "post-word" width arguments
// (FLG[1] + WDH)
int	int_disp3(long long nb, t_data *data)
{
	if (data->flag[1] == '-')
		return (ft_flood(data->wdh - int_len(nb), ' ', data->len));
	return (NO_ERROR);
}
